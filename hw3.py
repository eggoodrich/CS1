...
class Corpus():
    ...
    def updateFreqs(self, speech):
        for word in speech.wfreq.keys():
            if word in self.wfreq:
                self.dfreq[word] = self.dfreq[word] + 1
                self.wfreq[word] = self.wfreq[word] + speech.wfreq[word]
            else:
                self.dfreq[word] = 1
    ...
    def topK(self, k):
        L = [(word, self.wfreq[word]) for word in self.wfreq.keys() if self.wfreq[word] != len(self.speeches)]
        for i in range(k):
    ...
    def createVectors(self, k):
        ...
        for s in self.speeches.keys():
             self.speeches[s].makeVector(self.template, self.dfreq, len(self.speeches))
...
class Speech():
    ...
    def __init__(self, filename):
        ...
        # FIXME
        #
        self.words = [word.strip() for word in self.text.lower().split() if word != '.']
        ...
        for word in self.words:
            if word in self.wfreq and word not in SW:
                self.wfreq[word] = self.wfreq[word] + 1
            elif word not in SW:
                self.wfreq[word] = 1
...
if __name__ == '__main__':
    ...
    for p in P:
        for i in range(4):
            c.addSpeech(p + str(i))
    for u in U:
        print("Mystery speech {} is closest to: {}".format(u[-1], c.identify(u, 2000, 5)))
